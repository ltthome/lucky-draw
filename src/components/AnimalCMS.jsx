  ...
  const handleSave = async () => {
    const updates = animals.map((a) =>
      db.animals.update(a.id, {
        ...
        defaultTags: edits[a.id].defaultTags,
        defaultNotes: edits[a.id].defaultNotes,
      })
    )
  ...
  return (
    <div className="cms">
      ...
      <div className="cms-list">
        {animals.map((animal) => (
          <div key={animal.id} className="cms-card">
            ...
            <div className="cms-defaults">
              <input
                className="input cms-input-small"
                value={edits[animal.id]?.defaultTags || ''}
                onChange={(e) => handleChange(animal.id, 'defaultTags', e.target.value)}
                placeholder="Tags mặc định (vd: 1S, 2C)"
              />
              <input
                className="input cms-input-small"
                value={edits[animal.id]?.defaultNotes || ''}
                onChange={(e) => handleChange(animal.id, 'defaultNotes', e.target.value)}
